...
FairyBuffer::FairyBuffer(size_t size, vk::BufferUsageFlags buffer_usage, vk::MemoryPropertyFlags memory_property)
    : size_(size), host_pointer_(nullptr)
{
    ...
    vmaCreateBuffer(FairyContext::Get().allocator, &static_cast<VkBufferCreateInfo&>(buffer_create_info),
...
FairyBuffer::~FairyBuffer()
{
    vmaDestroyBuffer(FairyContext::Get().allocator, buffer_, allocation_);
}

vk::UniqueBufferView FairyBuffer::CreateBufferView(vk::Format buffer_format, VkDeviceSize offset,
                                                   VkDeviceSize range) const
{
    auto create_info = MakeBufferViewCreateInfo(buffer_format, offset, range);
    return FairyContext::Get().device.createBufferViewUnique(*create_info);
}

std::unique_ptr<vk::BufferViewCreateInfo>
FairyBuffer::MakeBufferViewCreateInfo(vk::Format buffer_format, VkDeviceSize offset, VkDeviceSize range) const
{
